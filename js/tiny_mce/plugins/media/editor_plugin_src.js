	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			ed.onPreInit.add(function() {
				...
				ed.schema.addValidElements('object[id|style|width|height|classid|codebase|*],param[name|value],embed[id|style|width|height|type|src|*],video[*],audio[*],source[*],track[*]');
		...
		dataToImg : function(data, force_absolute) {
			var self = this, sources, attrs, img, i, tracks;
			...
			tracks = toArray(data.video.tracks);
			if (tracks) {
				for (i = 0; i < tracks.length; i++) {
					tracks[i].src = self.convertUrl(tracks[i].src, force_absolute);
				}
			}
			...
			data.width = normalizeSize(data.width || (data.type == 'audio' ? "300" : "320"));
			if (typeof(data.width) == "string" && data.width.search(/[^0-9]/g) > 0) {
			...
			data.height = normalizeSize(data.height || (data.type == 'audio' ? "32" : "240"));
			if (typeof(data.height) == "string" && data.height.search(/[^0-9]/g) > 0) {
		...
		imgToObject : function(node, args) {
			var self = this, editor = self.editor, video, object, embed, name, value, data,
				...
				posterSrc, style, audio, tracks, track;
			...
			if (typeItem.name === 'Video' && data.video.sources[0]) {
				...
				tracks = data.video.tracks = toArray(data.video.tracks);
				for (i = 0; i < tracks.length; i++) {
					if (/\.vtt$/.test(tracks[i].src)) {
					}
				}

				/*if (!tracks[0].type) {
                    video.attr('src', tracks[0].src);
                    tracks.splice(0, 1);
                }*/

				for (i = 0; i < tracks.length; i++) {
					track = new Node('track', 1).attr(tracks[i]);
					track.shortEnded = true;
					video.append(track);
				}
		...
		objectToImg : function(node) {
			var object, embed, video, iframe, img, name, id, width, height, style, i, html,
				...
				hspace, vspace, align, bgcolor, tracks, track;
			...
			if (name === 'video' || name == 'audio') {
				...
				id = video.attr('id');
				data.video = {attrs : {}, sources : [], tracks : []};
				...
				// Get all tracks
				tracks = video.getAll("track");
				for (i = 0; i < tracks.length; i++) {
					track = tracks[i].remove();

					data.video.tracks.push({
						src: urlConverter.call(urlConverterScope, track.attr('src'), 'src', 'track'),
						kind: track.attr('kind'),
						srclang: track.attr('srclang'),
						label: track.attr('label'),
						default: track.attr('default'),
					});
				}
