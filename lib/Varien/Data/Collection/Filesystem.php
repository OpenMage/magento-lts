...
class Varien_Data_Collection_Filesystem extends Varien_Data_Collection
{
    ...
    protected function _usort($a, $b)
    {
        foreach ($this->_orders as $key => $direction) {
            ...
            return (self::SORT_ORDER_ASC === strtoupper((string) $direction) ? $result : -$result);
        }
    }
    ...
    public function addFieldToFilter($field, $cond, $type = 'and')
    {
        ...
        if (isset($cond['finset'])) {
            $filterValue = ($cond['finset'] ? explode(',', (string) $cond['finset']) : []);
    ...
    {
        $filterValueRegex = str_replace('%', '(.*?)', preg_quote((string) $filterValue, '/'));
        return (bool) preg_match("/^{$filterValueRegex}$/i", (string) $row[$field]);
    }
