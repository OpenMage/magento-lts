    ...
    public function send()
    {
        ...
        foreach ($collection as $message) {
            if ($message->getId()) {
                ...
                if ($parameters->getData('return_path_email') !== null) {
                    $mailTransport = new Zend_Mail_Transport_Sendmail("-f" . $parameters->getData('return_path_email'));
                ...
                if ($parameters->getData('is_plain')) {
                    $mailer->setBodyText($message->getMessageBody());
                } else {
                    $mailer->setBodyHtml($message->getMessageBody());
                }

                $mailer->setSubject('=?utf-8?B?' . base64_encode($parameters->getData('subject')) . '?=');
                $mailer->setFrom($parameters->getData('from_email'), $parameters->getData('from_name'));
                if ($parameters->getData('reply_to') !== null) {
                    $mailer->setReplyTo($parameters->getData('reply_to'));
                }
                if ($parameters->getData('return_to') !== null) {
                    $mailer->setReturnPath($parameters->getData('return_to'));
                }
