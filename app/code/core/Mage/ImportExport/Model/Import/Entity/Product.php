...
class Mage_ImportExport_Model_Import_Entity_Product extends Mage_ImportExport_Model_Import_Entity_Abstract
{
    ...
    protected function _importData()
    {
        ...
        Mage::dispatchEvent('catalog_product_import_finish_before', ['adapter' => $this]);
        return true;
    }
    ...
    protected function _initTypeModels()
    {
        ...
        foreach ($config as $type => $typeModel) {
            $model = Mage::getModel($typeModel, [$this, $type]);
            if (!$model) {
            ...
            foreach ($bunch as $rowNum => $rowData) {
                ...
                if ($typeSpecific[$type] === true && !empty($rowData['_custom_option_row_title'])) {
                    if (empty($rowData['_custom_option_store'])) {
                        ...
                        $customOptions[$typeValueTable][$prevOptionId][] = [
    ...
    protected function _prepareAttributes($rowData, $rowScope, $attributes, $rowSku, $rowStore)
    {
        ...
        foreach ($rowData as $attrCode => $attrValue) {
            ...
            if ($attribute->getBackendType() === 'datetime' && Carbon::parse($attrValue)->getTimestamp()) {
                $attrValue = gmdate(Varien_Date::DATETIME_PHP_FORMAT, Carbon::parse($attrValue)->getTimestamp());
