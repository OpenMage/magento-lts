        ...
        if ($this->getIsValid() && $this->getUserValue() !== null) {
            ...
            if ($this->_dateExists()) {
                ...
                $timestamp += Carbon::now()
                    ...
                    ->setMinute(0)
                    ->setSecond(0)
                    ->setMonth(Carbon::now()->format('m'))
                    ->setDay(Carbon::now()->format('d'))
                    ->setYear(Carbon::now()->format('Y'));
            }
            ...
            if ($this->_timeExists()) {
                ...
                $timestamp->addMinutes($value['minute']);
                $timestamp->addHours($value['hour']);
            }

            $date = new Zend_Date($timestamp);
            $result = $date->toString(Mage_Core_Helper_Date::DATETIME_INTERNAL_FORMAT);
