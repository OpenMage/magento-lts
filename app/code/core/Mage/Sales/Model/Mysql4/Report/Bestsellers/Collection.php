...
class Mage_Sales_Model_Mysql4_Report_Bestsellers_Collection extends Mage_Sales_Model_Mysql4_Report_Collection_Abstract
{
    ...
    protected function _initSelect()
    {
        ...
        if (!$this->_period) {
            ...
            if ($this->_from || $this->_to) {
                $select->from($this->getTable('sales/bestsellers_aggregated_daily'), $cols);
            } else {
                $select->from($this->getTable('sales/bestsellers_aggregated_yearly'), $cols);
            }
            $select
                ->group('product_id')
                ->order('qty_ordered DESC')
        ...
        if ('year' == $this->_period) {
            $select->from($this->getTable('sales/bestsellers_aggregated_yearly'), $this->_getSelectedColumns());
        } else if ('month' == $this->_period) {
            $select->from($this->getTable('sales/bestsellers_aggregated_monthly'), $this->_getSelectedColumns());
        } else {
            $select->from($this->getTable('sales/bestsellers_aggregated_daily'), $this->_getSelectedColumns());
        }
        if (!$this->isTotals()) {
     ...
     * @return Mage_Sales_Model_Mysql4_Report_Bestsellers_Collection
     */
