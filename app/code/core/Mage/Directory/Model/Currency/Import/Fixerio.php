...
class Mage_Directory_Model_Currency_Import_Fixerio extends Mage_Directory_Model_Currency_Import_Abstract
{
     ...
     * @var string
     */
    protected $_url = 'https://api.apilayer.com/currency_data/live?source={{CURRENCY_FROM}}&currencies={{CURRENCY_TO}}';
     ...
     * @return array
     */
    protected function _convertBatch(array $data, string $currencyFrom, array $currenciesTo): array
    {
        ...
        $url = str_replace(
            ...
            [$currencyFrom, $currenciesImploded],
            $this->_url
        );
        ...
        try {
            $response = $this->_getServiceResponse($url, $accessKey);
        } catch (Exception $e) {
        ...
        foreach ($currenciesTo as $currencyTo) {
            if ($currencyFrom == $currencyTo) {
                ...
                if (empty($response['quotes'])) {
                    ...
                    $data[$currencyFrom][$currencyTo] = $this->_numberFormat((float) $response['quotes'][$currencyFrom.$currencyTo]);
                }
            }
        }
        return $data;
     ...
     * @return array
     */
    protected function _getServiceResponse(string $url, $accessKey, int $retry = 0): array
    {
        $response = [];
        try {
            $jsonResponse = $this->_httpClient
                ->setUri($url)
                ->setHeaders(['Content-Type' => 'text/plain', 'apikey' => $accessKey])
