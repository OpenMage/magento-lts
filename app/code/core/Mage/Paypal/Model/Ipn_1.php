...
class Mage_Paypal_Model_Ipn
{
	...
	$receiver = ($this->getIpnFormData('business') ? $this->getIpnFormData('business') : $this->getIpnFormData('receiver_email'));
        if ($merchantEmail && $receiver && $merchantEmail != $receiver) {
            Mage::throwException(Mage::helper('paypal')->__('Requested %s and configured %s merchant emails do not match.', $receiver, $merchantEmail));
        }
    }
