...
class Mage_Adminhtml_Customer_AttributeController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function preDispatch()
    {
        ...
        parent::preDispatch();
        $this->_entityTypeId = Mage::getModel('eav/entity')->setType(Mage_Customer_Model_Customer::ENTITY)->getTypeId();
        return $this;
    }
    ...
    public function validateAction()
    {
        ...
        if ($attribute->getId() && !$attributeId) {
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('customer')->__('Attribute with the same code already exists'));
        ...
        if ($data) {
            ...
            if (isset($data['attribute_code'])) {
                ...
                if (!$validatorAttrCode->isValid($data['attribute_code'])) {
                    $session->addError(
                        Mage::helper('customer')->__('Attribute code is invalid. Please use only letters (a-z), numbers (0-9) or underscore(_) in this field, first character should be a letter. Do not use "event" for an attribute code.')
                    );
            ...
            if ($id) {
                ...
                if (!$model->getId()) {
                    $session->addError($helper->__('This Attribute no longer exists'));
                    $this->_redirect('*/*/');
                    return;
                }
                ...
                // entity type check
                if ($model->getEntityTypeId() != $this->_entityTypeId) {
                    $session->addError($helper->__('This attribute cannot be updated.'));
            ...
            try {
                $model->save();
                $session->addSuccess($helper->__('The product attribute has been saved.'));
                ...
                if ($this->getRequest()->getParam('popup')) {
                    $this->_redirect('adminhtml/customer/addAttribute', array(
    ...
    public function deleteAction()
    {
        if ($id = $this->getRequest()->getParam('attribute_id')) {
            ...
            if ($model->getEntityTypeId() != $this->_entityTypeId || !$model->getIsUserDefined()) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('customer')->__('This attribute cannot be deleted.'));
                $this->_redirect('*/*/');
                return;
            }
        ...
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('customer')->__('Unable to find an attribute to delete.'));
        $this->_redirect('*/*/');
    }
