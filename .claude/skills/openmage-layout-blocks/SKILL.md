---
name: openmage-layout-blocks
description: OpenMage layout XML, blocks, .phtml templates, design fallback chain, escaping discipline. Use when editing files under app/design/**/layout/*.xml, Block/**/*.php, or template/**/*.phtml; adding a block or template; working with layout handles like catalog_product_view; or fixing PHPStan errors in .phtml files.
---

# OpenMage Layout, Blocks, Templates
...
- `openmage-controllers-routing` — `loadLayout()`/`renderLayout()`, where layout handles get added, the dispatch flow.
- `openmage-translations` — `__()` resolution, the helper-vs-block translation distinction, `jstranslator.xml`.
- `mage-module-adminhtml` — admin grid/form/container/tabs block conventions, mass-actions, form key.
