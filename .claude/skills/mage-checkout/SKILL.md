...
3. **Hands off to sales**: `Mage::getModel('sales/service_quote', $this->getQuote())->submitAll()`. This is the single conversion entry point — see `mage-sales`.
...
- **Custom quote validation pre-place-order**: observe `sales_model_service_quote_submit_before` (in `mage-sales`) — `Mage_Core_Exception` aborts the order with a clean error.
...
- `mage-sales` — `Mage_Sales_Model_Service_Quote::submitAll()`, totals collectors, order/invoice/shipment lifecycle.
- `mage-customer` — login flow, `customer/session`, address book, `customer_address` EAV.
- `m1-events-observers` — for the persistent observer wall and `customer_login`/`checkout_submit_all_after` consumers.
- `m1-controllers-routing` — frontend router config, `_validateFormKey`, AJAX dispatch.
- `mage-payment-methods` — `savePayment` and order-place redirect for offsite gateways (Paypal Standard, etc.).
- `mage-promotions` — coupons (`_setCartCouponCode`) and totals interaction.
