...
See also: `m1-module-structure` (where these files live in a module), `m1-acl-adminhtml` (every section needs a matching ACL node or it won't render for non-root admins).
...
## ACL gating (cross-ref `m1-acl-adminhtml`)
...
7. If you added a brand-new section, add the matching ACL resource node (see `m1-acl-adminhtml`).
