---
name: m1-translations
description: Magento 1 translations — locale CSV files at app/locale/<locale>/Mage_<Module>.csv, __() helper/block translation, jstranslator.xml for JS strings, inline translation. Use when adding translatable strings, editing CSV files under app/locale/, working with __() in PHP/.phtml, or wiring jstranslator.xml.
---

# m1-translations

Magento 1 translations are per-module CSV files. Every translatable string flows through a `__()` call that resolves to the **module helper's** CSV. Missing translations fall through to the original string — no warning, no error.
...
- `m1-layout-blocks` — how blocks are constructed; `module_name` data key, `helper()` method.
- `m1-module-structure` — where `etc/jstranslator.xml` lives, module name → helper alias mapping.
