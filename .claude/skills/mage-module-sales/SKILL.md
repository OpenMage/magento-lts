...
Tax (`Mage_Tax`) and weee (`Mage_Weee`) inject themselves into this chain — see `mage-module-tax`. Catalog rules and sales rules inject `discount` / `freeshipping` collectors — see `mage-module-promotions`.
...
- `mage-module-checkout` — onepage / multishipping flow that *calls* `Service\Quote::submitAll`.
- `mage-module-payment-methods` — `Method_Abstract` `authorize`/`capture`/`void`/`refund` hooks invoked from invoice/creditmemo lifecycle.
- `mage-module-shipping-carriers` — rate result produces the `shipping_method` + `shipping_amount` consumed by `addressToOrder`.
- `mage-module-tax` — tax/subtotal/shipping_tax collectors slot into the quote totals chain.
- `mage-module-promotions` — `salesrule/quote_discount` collector and coupon application.
- `openmage-events-observers` — `sales_order_*`, `sales_order_invoice_*`, `sales_order_shipment_*`, `sales_order_creditmemo_*` event names; observe rather than rewrite.
- `openmage-db-setup-scripts` — adding a status: `Mage_Sales_Model_Order_Status::assignState` from a `data-upgrade-*.php`.
