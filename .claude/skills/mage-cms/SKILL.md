---
name: mage-cms
description: Magento 1 CMS — pages, static blocks, widgets, WYSIWYG/TinyMCE directives ({{block}}, {{store url}}, {{media url}}), Cms_Helper_Page routing, widget.xml declarations. Use when editing under Mage_Cms/ or Mage_Widget/, working with cms/* aliases, debugging WYSIWYG directive parsing, or registering a widget type.
---
...
- `m1-layout-blocks` — block aliases, layout handles, `_prepareLayout`. Widget instance binding leans on these.
- `m1-acl-adminhtml` — the split ACL tree above; `_isAllowed()` mechanics.
- `m1-caching` — `cms_page` / `cms_block` cache tags and FPC interaction.
- `m1-translations` — CMS content is *not* run through `__()` automatically; translate at authoring time or via a directive.
