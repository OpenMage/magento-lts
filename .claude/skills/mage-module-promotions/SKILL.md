...
- `conditions_serialized` → tree of `Mage_Rule_Model_Condition_Combine` (root) → `Combine | Condition_Abstract` (recursive). The root combine has an aggregator (`all`/`any`) and a boolean value (`1`/`0`, displayed as TRUE/FALSE); each leaf is a `field operator value` triple.
...
Subclasses must implement `getConditionsInstance()` and `getActionsInstance()` — they return the appropriate root combine / action collection for the rule type. CatalogRule: conditions = `catalogrule/rule_condition_combine`, actions = `catalogrule/rule_action_collection`. SalesRule: conditions = `salesrule/rule_condition_combine`, actions = `salesrule/rule_condition_product_combine` (a second condition tree, not a flat action list).
...
Storefront price lookup goes through observers on `catalog_product_get_final_price` → `Mage_CatalogRule_Model_Resource_Rule::getRulePrice()` → reads `catalogrule_product_price`. Configurable products additionally route through `calcProductPriceRule()` → `getRulesFromProduct()` against `catalogrule_product`. **Nothing happens at request time** beyond a table read.
...
2. If probability > `MAX_PROBABILITY_OF_GUESSING (0.25)`, **auto-grow `length`** until below threshold. The model rewrites its own `length` via `setLength()`. After `MAX_GENERATE_ATTEMPTS (10)` collisions per code, throws `Unable to create requested Coupon Qty`.
...
After a cart rule change: nothing to reindex. Next quote totals collection picks it up. There is no automatic cache invalidation on cart-rule save; the next quote totals collection re-evaluates rules.
...
- `mage-module-catalog` — the price index (`catalog_product_price`) that catalog rules feed; product collection joins for matched-by-rule.
- `mage-module-sales` — quote totals collection order; `Mage_SalesRule_Model_Quote_Discount` runs as one of the totals.
- `openmage-indexers-cron` — `catalog_product_price` indexer mechanics, `catalogrule_apply_all` cron job, `index_event` lifecycle.
- `openmage-events-observers` — `catalogrule_rule_save_commit_after`, `salesrule_rule_save_commit_after`, `catalog_product_get_final_price`.
