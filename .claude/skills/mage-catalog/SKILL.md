---
name: mage-catalog
description: Magento 1 Mage_Catalog — products (simple/virtual/configurable/bundle/grouped/downloadable), categories, URL rewrites, layered nav, flat tables, image cache, indexing chain. Use when editing under app/code/core/Mage/Catalog/, working with catalog/* aliases (catalog/product, catalog/category, catalog/url), adding product attributes, or debugging URL rewrites or layered nav.
---

# mage-catalog

Catalog is the largest M1 module and the EAV poster child. Almost every other commerce module hangs off it: sales/quote items reference `catalog/product`, promotions index against catalog rules, search reads from the catalog flat tables. Edits here ripple — be deliberate.

This skill covers the things you need to know to safely edit `Mage_Catalog`. For framework mechanics it leans on:
- `m1-eav` — attributes, source/backend models, the flat-table read switch
- `m1-indexers-cron` — `Mage_Index` event log, reindex modes, cron declarations
- `mage-product-types` — Bundle, Configurable+Swatches, Grouped, Downloadable internals
...
Real-time indexers update on save; manual indexers queue rows in `index_event` until a reindex runs. Toggle in System → Index Management. See `m1-indexers-cron` for the `Mage_Index` event log mechanics and the matrix code.
...
When you add a product attribute, decide whether it's needed in listings. If yes, set `used_in_product_listing = 1` in the EAV setup call and reindex `catalog_product_flat`. See `m1-eav` for the full flat pitfall.
...
**Add a product attribute.** EAV setup script under `Catalog/sql/catalog_setup/upgrade-X.Y.Z-X.Y.Z+1.php` calling `$installer->addAttribute('catalog_product', 'my_attr', [...])`. Bump `<modules><Mage_Catalog><version>` — **don't** bump core's version; create your own module if attributes belong to a third-party feature. If listing-visible, set `used_in_product_listing => 1` and reindex `catalog_product_flat`. See `m1-eav` and `m1-db-setup-scripts`.

**Add a category attribute.** Same flow with entity `catalog_category`. Reindex `catalog_category_flat`.

**Override a product type.** Don't subclass `Mage_Catalog_Model_Product_Type_Simple` — register a new type under `<global><catalog><product><type>` with a new id and your own type model. See `mage-product-types`.
...
- `m1-eav` — attribute setup, source/backend models, flat-table flags, store-scoped value semantics
- `m1-indexers-cron` — `Mage_Index` event log, matrix mode, cron declaration, shell/indexer.php
- `mage-product-types` — Bundle / Configurable / Grouped / Downloadable internals; option/selection/link/sample tables
- `mage-promotions` — catalog price rules read here; rule changes need a `catalogrule_product` reindex (separate indexer)
- `mage-tax` — final-price calculation hooks
- `m1-db-setup-scripts` — where to put EAV attribute installers
- `m1-events-observers` — `catalog_product_*`, `catalog_category_*`, `catalog_block_product_list_collection` are heavily observed
