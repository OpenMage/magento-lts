---
name: m1-module-structure
description: Magento 1 module manifest, etc/config.xml, class aliases, version bumps, area scoping. Use when creating or registering a module, editing app/etc/modules/*.xml or */etc/config.xml, bumping module versions, or wiring class aliases (catalog/product, sales/order). Triggers on Mage::getModel/getSingleton/helper alias resolution questions.
---

# Magento 1 Module Structure
...
The `<version>` here is what drives setup-script execution (see `m1-db-setup-scripts`). The version in `app/etc/modules/*.xml` is *not* read for setup — that file only controls activation. Bump the version in `etc/config.xml`, never the manifest.
...
The `<module>` value links the resource node to the `<modules>/<Mage_Catalog>/<version>` value. Every `etc/config.xml` version bump triggers a re-scan of `sql/<setup>/` and `data/<setup>/` for that module. See `m1-db-setup-scripts` for setup-script naming and the `Mage_Eav_Model_Entity_Setup` variant.
...
- **Controllers** are *not* PSR-0 autoloaded. `Mage_Catalog_ProductController` lives at `app/code/core/Mage/Catalog/controllers/ProductController.php` — note the lowercase `controllers/`. They're loaded by the dispatcher via classmap. See `m1-controllers-routing`.
- **Setup scripts** are loaded by filename, not class name. See `m1-db-setup-scripts`.
...
5. If shipping events: `<global><events>` (or area-scoped) — see `m1-events-observers`.
6. If shipping URLs: `<frontend><routers>` or admin controller registration — see `m1-controllers-routing`.
