...
Add attributes via setup scripts only — call `$installer->addAttribute('customer', 'my_attr', [...])` or `'customer_address'`. See `openmage-db-setup-scripts` and `openmage-eav`.
...
- **Tax** — `tax_class_id` on the group joins to `tax_class` and resolves via the customer-tax-class × product-tax-class matrix in `mage-module-tax`.
...
See `openmage-events-observers` for observer wiring.

## Cross-references

- `openmage-eav` — attribute creation, source/backend/frontend models, store-scoped values.
- `mage-module-checkout` — `customer/session` consumer; quote merging on login; persistent-cart hand-off.
- `mage-module-sales` — order address conversion from `customer_address`; `customer_group_id` carried onto the order.
- `mage-module-tax`, `mage-module-promotions` — group-driven calculation paths.
- `openmage-db-setup-scripts` — the only place to add EAV attributes.
- `openmage-system-config` — `customer/*` and `persistent/options/*` config trees.
