---
name: mage-shipping-carriers
description: Magento 1 shipping carriers — Mage_Shipping_Model_Carrier_Abstract, rate request/result, USA carriers (USPS/UPS/FedEx/DHL), tracking, system.xml allowed countries/methods, free shipping interaction. Use when editing under app/code/core/Mage/Shipping/ or app/code/core/Mage/Usa/, adding a carrier, modifying rate calculation, or wiring tracking info.
---

# mage-shipping-carriers
...
- `mage-sales` — quote address `collectShippingRates()`, `Mage_Sales_Model_Quote_Address_Rate` persistence, conversion to `sales_order_shipment`.
- `mage-promotions` — SalesRule "Free Shipping" action sets `$request->setFreeShipping(true)` and stamps per-item `free_shipping` flags.
- `m1-system-config` — backend/source/frontend models, scope behavior of the `carriers/*` config tree.
