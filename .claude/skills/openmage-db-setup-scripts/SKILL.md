---
name: openmage-db-setup-scripts
description: OpenMage install/upgrade scripts under sql/ and data/, version-bumped DB migrations, EAV attribute setup. Use when adding a column or table, writing a migration, bumping a module version, editing files under **/sql/*_setup/*.php or **/data/*_setup/*.php, or adding an EAV attribute.
---

# OpenMage DB Setup Scripts
...
Scripts are **immutable after release**. Once a version is in `core_resource` on any merchant's DB, that file will never run again on that DB. To change behavior, write a *new* script at a higher version and bump `<modules><Mage_Foo><version>` in `etc/config.xml`. See `openmage-module-structure` for where the version block lives.
...
- `openmage-module-structure` — where `<modules><version>` and the `<resources>/<setup>` wiring live, module activation manifest.
- `openmage-eav` — entity / attribute / set / group model; source/backend/frontend conventions for `addAttribute`.
